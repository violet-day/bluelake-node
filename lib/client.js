...
RpcClient.prototype.rpcInvoke = function (method, argMap) {
  ...
  request({
    ...
    if (err) {
      logger.error('requestId:%s,%s.%s,%s+ms,request:%j', blpBody.requestId, self.service, method, ms.toFixed(3), argMap);
      logger.fatal(err)
    } else {
      logger.info('requestId:%s,%s.%s,%s+ms,request:%j,response:%j', blpBody.requestId, self.service, method, ms.toFixed(3), argMap, data);
    }
  });
